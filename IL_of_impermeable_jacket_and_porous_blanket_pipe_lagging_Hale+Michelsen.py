...
# 该程序的使用方法：在本文件的“# 常量及隔声套的几何参数、材料参数、声波频带中心频率”一段里设置需要的变量;
  在终端(Terminal)里输入“python<"IL_of_impermeable_jacket_and_porous_blanket_pipe_lagging_Hale+Michelsen.py" >level_report.txt”,回车
...
# 常量及隔声套的几何参数、材料参数、声波频带中心频率、实测噪声等的输入
pi = 3.141592654  #圆周率
T = 20  #温度
c = 331 + 0.6*T  #空气声速
E = 69*10**9  #铝的杨氏模量 69*10**9 #钢的杨氏模量 212*10**9
...
noise_before_input = np.array([90,92,95,98,100,100,100]) #实测噪声，倍频程7分量向量（dB），如[71.1,67.3,73.9,84.1,91.5,93.2,88.4]；或单个A计权值（dBA），如[100]
control_valve_correction = np.array([20,16,17,9,6,5,7]) #equipment noise corrections
centrifugal_compressor_correction = np.array([15,12,9,7,3,10,12])
centrifugal_pump_correction = np.array([4,2,4,5,7,9,12])
reciprocating_compressor_correction = np.array([3,4,5,6,8,8,8])
equipment_noise_correction = control_valve_correction #选择连接设备，将A计权总值转线性谱修正值，可选项暂有上面4行中的项
...
d = D+2*l+2*h  #隔声套外径
B = E*h**3/(12*(1 - niu**2))  #弯曲刚度
m = rho*h  #金属壳的总面密度
c_L = 12**0.5/h*(B/m)**0.5  #铝壳中纵波速度
f_r = c_L/pi/d  #铝壳环频率
f_c = c*c/2/pi*(m/B)**0.5  #铝壳临界频率
...
while i <7 :
    if f[i] <300 :  # 频率在300Hz以下
        ...
        X_r[i] = (1000*(m/d)**0.5*(xi_r[i] - xi_r[i]**2)**0.25) - (2*d/(l*xi_r[i]))  #IL公式所需局部量
...
print(" ")
print("1. T , c , E , niu , rho , φ , l , h , Φ , m :")
print("  ",round(T,4),",",round(c,4),",",round(E,4),",",round(niu,4),",",round(rho,4),",",round(D,4),",",round(l,4),",",round(h,4),",",round(d,4),",",round(m,4))
print(" ")
